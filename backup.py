...
NEWLY_HIRED_FILE = "dataset/newly_hired.csv"
DISPLAY_WIDTH, DISPLAY_HEIGHT = 64, 64
NEWLY_HIRED_AREA = (17, 46)  # (width, height)
SCROLL_SPEED = 0.1  # Adjust scrolling speed
...
# Read newly hired usernames
def load_newly_hired():
    usernames = []
    try:
        with open(NEWLY_HIRED_FILE, newline="", encoding="utf-8") as csvfile:
            reader = csv.DictReader(csvfile)
            for row in reader:
                if "name" in row:
                    usernames.append(row["name"].strip())
    except FileNotFoundError:
        pass
    return usernames

# Draw scrolling newly hired text
def draw_newly_hired(display_image, usernames, offset):
    draw = ImageDraw.Draw(display_image)
    y_position = DISPLAY_HEIGHT - NEWLY_HIRED_AREA[1] - offset
    for username in usernames:
        draw.text((NEWLY_HIRED_AREA[0], y_position), username, font=font, fill="white")
        y_position += FONT_SIZE + 2  # Spacing
        if y_position > DISPLAY_HEIGHT:
            break  # Stop drawing if out of display bounds

def scroll_newly_hired():
    usernames = load_newly_hired()
    if not usernames:
        return
    ...
    while True:
        ...
        draw_newly_hired(display_image, usernames, offset)
        display_image.show()  # Replace with actual display update function
        
        offset += 1
        if offset > (len(usernames) * (FONT_SIZE + 2)):
            offset = 0  # Reset scroll after full cycle
        
        time.sleep(SCROLL_SPEED)

@app.route("/update_newly_hired", methods=["POST"])
def update_newly_hired():
    data = request.json
    ...
    if action == "add" and username not in usernames:
        usernames.append(username)
    elif action == "remove" and username in usernames:
        usernames.remove(username)
    
    with open(NEWLY_HIRED_FILE, "w", newline="", encoding="utf-8") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["name"])
        for name in usernames:
            writer.writerow([name])
    
    return jsonify({"message": "Updated successfully", "newly_hired": usernames})
